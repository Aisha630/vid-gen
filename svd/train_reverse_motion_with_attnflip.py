...
def main():
    ...
    image_encoder = CLIPVisionModelWithProjection.from_pretrained(args.pretrained_model_name_or_path, subfolder="image_encoder", variant=args.variant)
    vae = AutoencoderKLTemporalDecoder.from_pretrained(args.pretrained_model_name_or_path, subfolder="vae", variant=args.variant)
    unet = UNetSpatioTemporalConditionModel.from_pretrained(args.pretrained_model_name_or_path, subfolder="unet", low_cpu_mem_usage=True, variant=args.variant)
    ...
    train_dataset = StableVideoDataset(video_data_dir=args.train_data_dir, max_num_videos=args.max_train_samples, num_frames=args.num_frames, is_reverse_video=True, double_sampling_rate=args.double_sampling_rate)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                ref_model_pred = ref_unet(noisy_latents_ref_inp.to(weight_dtype), timesteps.to(weight_dtype), encoder_hidden_states=encoder_hidden_states_ref, added_time_ids=added_time_ids, return_dict=False)[0]
                model_pred = unet(noisy_latents_inp, timesteps, encoder_hidden_states=encoder_hidden_states, added_time_ids=added_time_ids, return_dict=False)[0]  # v-prediction
                # Denoise the latents
            ...
            if args.validation_data_dir is not None and epoch % args.validation_epochs == 0:
                ...
                with torch.cuda.amp.autocast():
                    for val_idx in range(num_validation_images):
                        ...
                        videos.append(pipeline(ref_unet=ref_unet, image=val_img, ref_image=val_img, num_inference_steps=50, generator=generator, output_type="pt").frames[0])
